...
class MainWindow(CTk):
   def __init__(self):
       ...
       try:
           ...
           #self.sock.connect(('6.tcp.eu.ngrok.io', 10258))
   ...
   def toggle_show_menu(self):
       if self.is_show_menu:
           ...
           self.saveNameBtn.pack()

   def saveName(self):
       self.add_message(f"Користувач {self.username} змінив нік на {self.entry.get()}")
       message = f"Користувач{self.username} змінив нік на{self.entry.get()}"
